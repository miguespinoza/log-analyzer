...
import { dedupeLogLines, searchLines, sortLines } from "./log-lines-domain";
...
test("should search lines with filters", () => {
  ...
  const result = searchLines(fileA.getLogLines(), false, [filter]);
...
test("should search lines with filters and hide all the lines that are not metched by a filter", () => {
  ...
  const result = searchLines(fileA.getLogLines(), true, [filter]);
...
Wed Sep 28 2022 13:01:27 GMT-0700 (Pacific Daylight Time) <912> -- info -- ...log`;
  ...
  const sorted = sortLines("date", "desc", file.getLogLines(), [file]);
